...
Use the package that owns the required utility. For example, a `Copy` bound produces an independent logical value:

```mbt check
fn[T : @copy.Copy] duplicate(value : T) -> (T, T) {
  (@copy.Copy::copy(value), @copy.Copy::copy(value))
}
