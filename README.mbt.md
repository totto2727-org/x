...
JSON flattening is provided by `totto2727/x/json`. Both `flatten` and `from_flatten_json` require either RFC 6901 `JsonPointer` paths or JavaScript-style property and array-index paths. Values remain `Json`; reconstruction rejects object-valued leaves, conflicting paths, sparse arrays, paths over 16,384 characters, and paths deeper than 256 segments. Flattening rejects nested empty objects, and JSON Pointer rejects object keys matching RFC 6901 array-index syntax because they are indistinguishable from array indices without an existing document; use JavaScript bracket notation when those keys must round-trip.
