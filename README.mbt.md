...
JSON flattening is provided by `totto2727/x/json`. Both `flatten` and `from_flatten_json` require either RFC 6901 `JsonPointer` paths or JavaScript-style property and array-index paths. Values remain `Json`; reconstruction rejects object-valued leaves, conflicting paths, and sparse arrays. Flattening rejects nested empty objects, and JSON Pointer rejects numeric object keys because they are indistinguishable from array indices without an existing document; use JavaScript bracket notation when numeric keys must round-trip.
